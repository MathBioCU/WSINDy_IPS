...
%% nonlocal forces - Morse potentials
sig1 = 0.1;
sig2 = 1;
str = 0.2;
fx = @(x,y) -str*x./max(eps,hypot(x,y)).*[exp(-hypot(x,y)/sig1)/sig1 - exp(-hypot(x,y)/sig2)/sig2];
fy = @(x,y) -str*y./max(eps,hypot(x,y)).*[exp(-hypot(x,y)/sig1)/sig1 - exp(-hypot(x,y)/sig2)/sig2];
