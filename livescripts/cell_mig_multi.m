...
flz = {'20240604/r02c09f09_ROI1.mat',... %1.5
       '20240604/r03c11f05_ROI1.mat',... % 1.5
       '20240604/r02c09f08_ROI1.mat',... % 2
       '20240604/r02c09f09_ROI2.mat',... % 2
       '20240604/r03c11f05_ROI2.mat',... % 2
       '20240604/r03c11f06_ROI1.mat',... % 2
       };
...
for ff = 1:length(flz)
    ...
    % non-local operators
    dx = 10;%mean(diff(xs_obs{1}));
    convargs = {{'dimx',dim-1,'utagin',1,'utagout',1,'psitags',1,'Mon',[],'Sing',[],'Exp',[1./(2*dx:2*dx:40*dx)],'Singeps',[0.01],'svdtol',[1e-4]}};
    ...
    p_x = 5; mxs = floor(length(xs_obs{1})/4);
    tau_x = []; k_x = []; tauhat_x = []; 
    p_t = 3; mts = 15;
    tau_t = []; k_t = []; tauhat_t = [];
...
maxits = Inf;
sparsity_scale = 1;
excl_tags= {'([x.^0.*y.^0.*t.^0]u^1)_{lap}',['u^{',num2str(max(polys)),'}_{}']};
excl_tols = [0.000001 -0.1];
excl_signs = [-1 1];
